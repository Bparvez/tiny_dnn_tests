 ...
 * function.
 */
template <typename N> void construct_net(N &nn, tiny_dnn::core::backend_t backend_type) {
...
/*
 * @brief Training the network.
 */
void train_cifar10(std::string data_dir_path, double learning_rate,
                   const int n_train_epochs, const int n_minibatch,
                   tiny_dnn::core::backend_t backend_type, std::ostream &log) {
...
/*
 * @brief Parse name of the backend from the commandline
 *
 * @note By default we have been using internal
 * @note TODO: Try different backend, a point for optimizations ?, why is the
 * backend only used during training
 */
...
/*
 * @brief Throws usage error when using from command line
 *
 * @note Can be hardcoded or left as they are, TODO: Decide after further
 * training
 */
